# Quálitas Controladora (Q.MX) — ficha al 2026-09-25

> BMV (Q*) · Servicios financieros / seguro de autos · Moneda de reporte: MXN (criterios contables CNSF) · Disponible en SIC: no aplica (emisora mexicana; cotiza en BMV) · Estado de conocimiento: Documentado
> Precio: Ps 152.79 (cierre 24-sep-2026). Rango de cierres en 52 semanas: 149.00-191.09. Precio −7.8% a 1 año, sin dividendos (Yahoo con ajuste: −2.5%; ese ajuste no está verificado para .MX). Bajo su media de 200 días (−8.1%) [10]. Fase 0: investigación, no recomendación.

## 1. Negocio y segmentos
Es el líder del seguro de autos en México: 34.2% de la prima emitida, 37.4% de la devengada y 45.4% en equipo pesado (AMIS, 1T26). Asegura 6.14 millones de unidades [1][2].

**Prima emitida del 1S26 por segmento (Ps 39,022 M)** [1]:
- Tradicional 63%: individual 37% y flotillas 26%.
- Instituciones financieras 32%.
- Subsidiarias en el extranjero ~5%.

**Geografía:** México aporta ~95% de la prima. El resto viene de Costa Rica, Perú, El Salvador, Colombia (20 oficinas) y EUA, este último en salida del negocio de camiones transfronterizos [1].

**Otros negocios:** verticales de refacciones, salvamentos y cristales, con Ps 456 M de ventas en el 2T26 [1].

**Utilidad:** técnica más financiera. El portafolio es de Ps 53,430 M: 85.7% renta fija con duración de 2.6 años, 14.3% renta variable y ~21% en USD [1][2].

## 2. Último reporte: 2T26 (21-jul-2026) [1][2]
- **Prima emitida:** Ps 17,329 M (−0.5% a/a). Sin el cambio de cobertura de una cuenta multianual habría crecido +3.4%.
- **Prima devengada:** Ps 17,392 M (+4.4%).
- **Índice combinado:** 95.7% (+293 pb). Siniestralidad 64.8%, adquisición 25.1% y operación 5.8%.
- **Margen operativo:** 4.7% de la devengada.
- **RIF:** Ps 1,161 M, con RSI de 7.4%.
- **Utilidad neta:** Ps 1,389 M (−1.3%).
- **UPA:** Ps 3.47 (utilidad ÷ 400 M acciones), contra un consenso de 3.84 [9].
- **Ingresos:** TradingView y Investing reportan 18.55 mil M, que equivale a prima devengada más RIF. El consenso era de 19.14-19.20 mil M [9].
- **Guía 2026 (ene-2026)** [4]:
  - Prima emitida de un dígito alto a dos dígitos bajos.
  - Siniestralidad dentro o un poco arriba del rango de 62-65%.
  - Índice combinado en el extremo superior de 92-94% o un poco arriba.
  - RIF "consistente" con 2025.
  - En julio se reiteró un ROE de largo plazo cercano o superior a 20%, "incluyendo 2026" [2].
- **Datos operativos:**
  1. Frecuencia de 6.6% (−23 pb a/a).
  2. Robos −15.3%, con recuperación de 49.4% contra 43.2% de la industria.
  3. El IVA pesó ~320-340 pb en la siniestralidad del 1S26 [1][2].

## 3. Tabla financiera
Trimestral (Ps M). La UPA es la utilidad neta entre 400 M de acciones, la misma convención de TradingView [3]:

| Trim | Prima emitida | Prima devengada | Índice combinado* | Margen oper.** | RIF | Utilidad neta | UPA |
|---|---|---|---|---|---|---|---|
| 3T24 | 16,151 | 15,321 | 95.6% | 2.9% | 1,183 | 1,145 | 2.86 |
| 4T24 | 20,898 | 16,610 | 90.1% | 3.5% | 1,557 | 1,352 | 3.38 |
| 1T25 | 18,822 | 16,833 | 88.2% | 8.6% | 1,533 | 2,145 | 5.36 |
| 2T25 | 17,414 | 16,666 | 92.8% | 5.9% | 1,218 | 1,407 | 3.52 |
| 3T25 | 17,335 | 17,802 | 93.0% | 7.8% | 1,134 | 1,734 | 4.33 |
| 4T25 | 22,233 | 18,022 | 102.6% | −8.6% | 1,225 | −190 | −0.48 |
| 1T26 | 21,693 | 18,800 | 90.2% | 5.5% | 1,176 | 1,555 | 3.89 |
| 2T26 | 17,329 | 17,392 | 95.7% | 4.7% | 1,161 | 1,389 | 3.47 |

\*Es el índice de la emisora: adquisición sobre prima retenida, siniestros sobre devengada y operación sobre emitida.
\*\*Resultado operativo entre prima devengada.

Anual [3][4]:

| Año | Prima emitida | Índice combinado | Margen oper. | Utilidad neta | UPA | Capital | ROE |
|---|---|---|---|---|---|---|---|
| 2021 | 38,225 | 91.7% | 6.9% | 3,778 | 9.31 | 20,379 | 19.2% |
| 2022 | 42,036 | 96.4% | 2.6% | 2,209 | 5.52 | 19,519 | 11.1% |
| 2023 | 53,726 | 96.1% | 1.9% | 3,791 | 9.48 | 21,445 | 18.4% |
| 2024 | 69,276 | 92.1% | 4.6% | 5,119 | 12.80 | 24,667 | 22.1% |
| 2025 | 75,804 | 94.1% | 3.3% | 5,095 | 12.74 | 25,716 | 20.2% |

Notas a la tabla anual:
- **Deuda:** no tiene deuda financiera ("financiamientos obtenidos" y "emisión de deuda" están en cero) [1].
- **Acciones:** 400 M. En 2021 eran 406 M; al 2T26 hay ~6.2 M en tesorería [1][3].
- **FCF y SBC:** no se revelan (n.d.). Para una aseguradora, el FCF equivale a la variación de reservas más la utilidad.

## 4. Expediente de 10 puntos
1. **Utilidad contra efectivo.** Las primas por cobrar suben más rápido que la emisión: Ps 49,737 M (+17.5% a/a) contra +7.7% de prima emitida en el 1S26. Los adeudos de dependencias federales pasaron de Ps 94 M a 1,365 M. Las reservas técnicas suben +12.5% [1]. *Inferencia:* hay más prima financiada y más cobro al gobierno; hay que vigilar la estimación para castigos (Ps 194 M).
2. **Deuda.** No tiene. La solvencia es de 341%: margen de Ps 15,984 M sobre un requerimiento de 6,646 M [1].
3. **Dilución.** No hay. Tiene recompra con un fondo de Ps 800 M para 2026 [6].
4. **Capital de trabajo.** Ver el punto 1. Las reservas (Ps 69,240 M) dan el float del que sale el RIF.
5. **Competidores.** GNP, AXA, Chubb, HDI, Mapfre y las aseguradoras de los bancos. La dirección describe una competencia de precios "profunda y agresiva" [2].
6. **Concentración.** El canal de instituciones financieras (automotrices y bancos) pesa 32%, y una cuenta multianual grande movió el trimestre. No se revela el peso de cada cliente [1].
7. **Regulación.** Desde el 1-ene-2026 el IVA de los proveedores de siniestros no es acreditable (Ley de Ingresos 2026). La emisora estimó ~Ps 2,000 M de impacto no recurrente en 2025. El real fue de ~Ps 1,683 M: utilidad de 6,778 sin IVA contra 5,095 reportada [4][7]. La solvencia la supervisa la CNSF.
8. **Promesas contra resultados.** En el 1S26 [1][4]:
   - **Cumple la guía:** prima emitida +7.7% e índice combinado de 92.8%.
   - **No cumple:** el RIF cae −15% (2,337 contra 2,751), contra un RIF "consistente", y el ROE de 12 meses es de 18.3%, contra "≥20%".
9. **Partes relacionadas.** 5 de los 12 consejeros son relacionados, incluidos el presidente ejecutivo y el director general. Los negocios verticales se consolidan [6]. No se verificaron los montos de operaciones con partes relacionadas.
10. **Asignación de capital.**
    - **Dividendo:** Ps 3,600 M (Ps 9.00 por acción), pagado en dos partes: el 13-may y el 5-nov-2026 [6].
    - **Destino del resto:** Colombia, verticales y recompra.

## 5. Cadena causal
**A. IVA no acreditable en siniestros.**
- **Exposición:** los siniestros son ~64% de la prima devengada.
- **Efecto económico:** +320-340 pb de siniestralidad que se trasladan solo en parte a la tarifa. La prima promedio sube "cerca de la inflación" [4].
- **Estado financiero:** combinado en el extremo alto del rango; ROE de 12 meses de 18.3%.
- **Valuación:** el P/VL bajó de 2.89x (dic-2025) a 2.41x.
- **Qué descuenta el precio:** *Inferencia:* el precio ya incorpora ~2 pp menos de ROE estructural.

**B. Guerra de precios con un PIB de ~1%** [2].
- **Exposición:** 63% de la prima es tradicional (flotillas e individual).
- **Efecto económico:** la emisión se estanca (−0.5% en el 2T26) aunque las unidades crecen +2%; la prima por unidad baja.
- **Estado financiero:** la devengada se desacelera con retraso de 2-4 trimestres, porque los costos se ajustan después de la emisión.
- **Valuación:** menor crecimiento de UPA 2027.

**C. Tasas y peso.**
- **Exposición:**
  - Banxico en 6.50% contra 8.0% en el 2T25; RSI de 7.4% contra 8.4% [1][11].
  - ~21% del portafolio está en USD, y cada peso de movimiento en el USD/MXN vale ~Ps 665 M al año [2].
- **Efecto económico:** el peso promedió 17.23 en el 3T26, contra 17.39 en el 2T26 [10].
- **Estado financiero:** RIF plano o a la baja.
- **Valuación:** le quita soporte a la utilidad.

## 6. Qué descuenta el precio
Capitalización de ≈Ps 60,258 M (394.4 M acciones netas de tesorería × 152.79).

**Múltiplos actuales:**
- **P/U de 12 meses:** 13.4x (utilidad de 12 meses de Ps 4,488 M, con la pérdida del 4T25 por IVA).
- **P/U sobre el 1S26 anualizado:** 10.2x (Ps 5,888 M).
- **P/VL:** 2.41x.
- **Dividendo:** 5.9%.

**Historia de 5 años al cierre de cada año (400 M acciones; 406 M en 2021):**

| Año | P/U | P/VL |
|---|---|---|
| 2021 | 11.2x | 2.07x |
| 2022 | 15.6x | 1.77x |
| 2023 | 18.1x | 3.21x |
| 2024 | 13.6x | 2.83x |
| 2025 | 14.6x (11.0x sin IVA) | 2.89x |

El P/VL actual está debajo del promedio de 2.55x.

**Pares (Yahoo, no verificado):**

| Emisora | P/U | P/VL | ROE |
|---|---|---|---|
| Progressive | 10.2x | 3.42x | 34.9% |
| Porto Seguro | 8.3x | 1.87x | 24.2% |

**Precio objetivo:** consenso Bloomberg de Ps 182.40, con un rango de 157-212 entre 12 corredores [8].

**Ingreso residual inverso (supuestos explícitos):** k_e de 12% en MXN (beta menor a 1) y g de 5%. Con P/VL = (ROE − g)/(k_e − g), el ROE implícito es **~21.9%**. Queda arriba del ROE de 12 meses (18.3%) y cerca del ROE del periodo (21.4%). *Inferencia:* el precio descuenta que la dirección cumple su meta de ROE de ~20% o más. Si el ROE se estabiliza en 18%, el P/VL justo es ~1.9x (−21%).

## 7. Escenarios de negocio a 12 meses
- **Bajista (30%):** la guerra de precios se profundiza, la prima emitida de 2026 crece menos de 5% y la siniestralidad del 2S supera 66% por lluvias y costo medio. El ROE 2026 queda abajo de 17%.
- **Base (50%):** emisión +5-8%, combinado de 93-95%, RIF de ~Ps 4,600 M y ROE de 19-21%.
- **Alcista (20%):** la competencia se racionaliza, la frecuencia se mantiene baja y el combinado baja de 92%. El ROE supera 22%.
- **Qué haría cambiar de escenario:** el crecimiento de la prima emitida del 3T26 (+5% o más apunta al alcista; negativo, al bajista) y la siniestralidad del 3T26 (más de 67% apunta al bajista).

## 8. Pronósticos registrados (3T26)
**Fecha estimada:** 20-21-oct-2026 (no confirmada).
- Yahoo: 20-oct. TradingView e Investing: 15-oct [9].
- Patrón de la emisora: el 3T25 se reportó el 21-oct-2025 y el 2T26 el 21-jul-2026 [1][5].

**Ingresos del 3T26 (prima devengada + RIF, definición de TradingView):**
- **Punto:** Ps 19,700 M. **Intervalo de 80%:** 18,900-20,350.
- **Consenso:** Ps 19,220 M (TradingView y Yahoo, 6 analistas; 25-sep-2026). La definición del proveedor no es homogénea.
- **Supuestos:** devengada de ~18,550 (+4% a/a sobre 17,802) y RIF de ~1,150.

**Prima emitida del 3T26 (dato de referencia de la emisora):**
- **Punto:** Ps 17,850 M (+3% a/a). **Intervalo de 80%:** 17,000-18,800.
- **Consenso:** no hay consenso homogéneo verificado.

**UPA del 3T26:**
- **Punto:** Ps 3.35 (utilidad de ~Ps 1,340 M). **Intervalo de 80%:** 2.75-3.95.
- **Consenso:** Ps 3.53 (TradingView y Yahoo; Investing 3.44) [9].
- **Supuestos:** combinado de ~95.8%. El 3T es temporada de lluvias; se suman ~330 pb por IVA a una base de 62.4% en el 3T25. Tasa efectiva de 30%.

**Binarios:**
1. Ingresos (devengada + RIF) del 3T26 mayores a Ps 19,220 M: **p = 0.70**.
2. UPA del 3T26 mayor a Ps 3.53: **p = 0.37**.

## 9. Catalizadores y riesgos
**Catalizadores:**
- Oct: datos de la AMIS y ventas de autos de la AMDA.
- 20 y 31-oct: Ley de Ingresos 2027 [11].
- ~20-oct: reporte del 3T26.
- 5-nov: pago del segundo dividendo (Ps 4.50).
- 5-nov y 17-dic: decisiones de Banxico.
- Ene-2027: guía 2027.

**Riesgos:**
- Guerra de precios.
- Huracanes y lluvias en sep-oct.
- Costo de refacciones y mano de obra, por el salario mínimo.
- Tasas más bajas.
- Cobranza a dependencias.
- Desaceleración de las ventas de autos.

## 10. Fuentes
1. Quálitas, Reporte Trimestral 2T26 (21-jul-2026): https://qinversionistas.qualitas.com.mx/storage/informes/2026/trimestral/2T/mx/Q%20-%20Reporte%20Trimestral%202T26%20VFF2.pdf
2. Quálitas, guion de la llamada del 2Q26 (22-jul-2026): https://qinversionistas.qualitas.com.mx/storage/informes/2026/trimestral/2T/Script%202Q%202026%20VF_SL_VF.pdf
3. Quálitas, Datos Financieros Históricos al 2T26 (xlsx): https://qinversionistas.qualitas.com.mx/storage/informes/2026/trimestral/2T/mx/DatosFinancierosHistoricos%202T26.xlsx
4. Quálitas, Reporte 4T25 y 2025 con expectativas 2026 (28-ene-2026): https://qinversionistas.qualitas.com.mx/uploads/informes/2025/trimestral/4T/mx/Q%20-%20Reporte%20Trimestral%204T25%20y%202025.pdf
5. Quálitas, Reporte 3T25 (21-oct-2025): https://qinversionistas.qualitas.com.mx/uploads/informes/2025/trimestral/3T/mx/Q-Reporte_Trimestral_3T25.pdf
6. Quálitas, Puntos aprobados de la AGO (29-abr-2026): https://qinversionistas.qualitas.com.mx/storage/informes/2026/trimestral/1T/15.Puntos%20Aprobados_AGA_2026.pdf
7. Quálitas, evento relevante sobre la Ley de Ingresos 2026 y el IVA (10-nov-2025): https://qinversionistas.qualitas.com.mx/default/pdf/eventos-relevantes/2025/Q-Aprobaci%C3%B3n%20de%20la%20Ley%20de%20Ingresos%202026.pdf
8. Quálitas RI, cobertura de analistas y consenso Bloomberg (consulta del 25-sep-2026): https://qinversionistas.qualitas.com.mx/ES/cobertura-de-analistas
9. Consenso y fechas (25-sep-2026): TradingView https://www.tradingview.com/symbols/BMV-Q/forecast/ · Investing https://www.investing.com/equities/qualitas-controladora-sab-de-cv-earnings · Yahoo Finance quoteSummary Q.MX
10. Yahoo Finance, chart de Q.MX, MXN=X, PGR y PSSA3.SA (25-sep-2026): https://query1.finance.yahoo.com/v8/finance/chart/Q.MX · `empresas/Q.MX/dossier-2026-09-25.md`
11. Conocimiento interno: `conocimiento/16-macro-global-divisas-y-el-peso.md` (Banxico 6.50%) · `24-politica-publica-regulacion-y-mercados.md` §4.7-4.8 · `03-maestria-valuacion-y-analisis-fundamental.md` §2.3 y §2.6
