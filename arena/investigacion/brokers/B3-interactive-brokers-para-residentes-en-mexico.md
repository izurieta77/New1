...
**Relación con documentos previos:** no repito A1 (`01-gbm-operativa-y-costos.md`), A2 (`02-universo-sic-bmv-agresivo.md`) ni B1. De B1 tomo la línea base: 0.29% por lado con IVA, desviación cambiaria implícita del SIC de ~0.1%–0.25% por lado, TC de 17.71 MXN/USD del 24-sep-2026 y el patrón de operación de la cuenta (≤1–1.5 rotaciones al mes, órdenes de 5,000 a 10,000 MXN). B1 dejó abierto el régimen fiscal de una venta en NYSE/Nasdaq (§5 y §10 de B1). Aquí lo avanzo con el texto de la LISR, sin cerrarlo. `conocimiento/11-mexico` no existe en el repo. `laboratorio/replicas/V05-*` solo tiene el pre-registro; su punto 5 (impuesto sucesorio) se responde aquí con fuente del IRS.
...
9. **[H] El universo se abre.** Ya aparece en la tabla maestra (filas 18–28) y en §7:
   ...
   - La regla PDT de 25,000 USD ya no existe; la cuenta, debajo de 2,000 USD, operaría como cuenta de efectivo [15].
...
12. **[H/I] Impuestos en México. Es el punto débil de IBKR para el dinero real del dueño (no para el TWR del torneo).**
    - IBKR no retiene ISR mexicano ni emite constancia mexicana. Entrega Form 1042-S y un reporte anual de dividendos [19].
    - La venta en NYSE/Nasdaq **no entra en las fracciones I–II del art. 129** (10% definitivo), que exigen bolsa concesionada en México [23].
    - [I] Cae en el régimen general de enajenación de bienes: acumulable a tarifa progresiva (art. 120). El art. 126 manda un **pago provisional por operación dentro de 15 días** cuando el adquirente es extranjero [23].
    - Los **dividendos extranjeros se acumulan y además pagan 10% adicional definitivo a más tardar el día 17 del mes siguiente** (art. 142 fr. V) [23].
    - **No verificado con el SAT ni con un contador.**
13. **[R] Veredicto para el frente B3:**
    ...
    - **Para el dinero neto del dueño después de impuestos, la ventaja se reduce o se invierte** si su tasa marginal es alta y la cuenta rota mucho (§10.4), y suma carga administrativa: pagos provisionales y pagos mensuales por dividendos.
    - La decisión final se toma contra B1 y B2. Hay que confirmar el régimen fiscal con un contador antes de mudar la cuenta.
...
| 6 | Comisión acciones/ETFs EUA, Tiered (≤300,000 acc./mes) | 0.0035 USD/acción; mín. 0.35 USD; máx. 1% (una nota al pie dice que Tiered se topa en 0.5%); + bolsa, compensación, traspaso, regulatorias | — | H | [1] |
...
| 28 | Regla PDT (25,000 USD) | Eliminada; margen y cortos requieren 2,000 USD | N/A | H | [15] |
...
| 34 | Régimen de ganancias en México | **Fuera del art. 129** (venta fuera de bolsa concesionada) → régimen general, acumulable, pagos provisionales | 10% definitivo (art. 129) | I (sobre texto legal) | [23] |
...
| Fracciones | Mismas tarifas; mínimo 0.01 USD; ej. 50% de acción de 10 USD = 0.05 USD | Igual |
...
- **(no verificado)** Si la conversión automática funciona con MXN como moneda base en una cuenta de efectivo, y si las ventas regresan a MXN solas o se quedan en USD. Hay que probarlo con la primera operación.
...
| E. Wise, de MXN a USD por transferencia bancaria (ACH) hacia IBKR | 0 | **193.84 MXN** (0.86% + 23.51 MXN) por 20,000 MXN; recibe 1,118.09 USD al TC medio de 17.714 (cotización del 25-sep-2026) | **0.97%** | H [29] |
...
- **[H] PDT:** FINRA RN 26-10: la SEC aprobó el 14-abr-2026, entró en vigor el 4-jun-2026 y los brókers tienen hasta el 20-oct-2027 para implementarla. Elimina el mínimo de 25,000 USD y el conteo de *day trades* [15]. IBKR: "the $25,000 minimum equity requirement for pattern day traders is no longer required... the minimum equity requirement is $2,000 to trade with leverage or execute short sales" [15]. **[I]** Con ~1,129 USD la cuenta no tiene margen y opera como cuenta de efectivo. Aplican las reglas de liquidación (T+1) y de *free-riding* de Reg T [S][33].
- **(no verificado)** ForecastTrader (contratos de eventos de ForecastEx) para residentes en México. El aviso oficial dice que "Forecast Contracts on US election results are only available to eligible US residents" [6]. Queda pendiente como posible alternativa a Kalshi.
...
- **[H]** El art. 129 (10% definitivo) cubre:
  - Fr. I: acciones mexicanas o "acciones emitidas por sociedades extranjeras cotizadas en dichas bolsas de valores" cuando "su enajenación se realice en las bolsas de valores concesionadas... en los términos de la Ley del Mercado de Valores".
  ...
  - La suma de ganancias se hace por intermediario, incluidas "entidades financieras extranjeras con los que tenga un contrato de intermediación" [23]. Eso vale para la fr. III (emisoras mexicanas), no para acciones de EUA.
- **[I] Una venta de AAPL o SPMO en Nasdaq/NYSE por IBKR no cabe en las fracciones I–III.** Iría al régimen general de enajenación de bienes (Título IV, Cap. IV):
  ...
  - Si el art. 126 aplica al pie de la letra, **cada venta generaría un pago provisional en 15 días**. El Reglamento permite un pago menor ("excepto en los casos en los que el enajenante manifieste... que efectuará un pago provisional menor... requisitos que señale el Reglamento") [23]. **No revisé el RLISR ni la RMF 2026.**
...
- **(no verificado con el SAT ni con un contador):** el régimen exacto, cómo se compensan las pérdidas en esta ruta, si existe una regla miscelánea que simplifique los pagos provisionales de ventas en mercados reconocidos extranjeros, y la tasa marginal máxima de la tarifa del art. 152. La tabla del PDF no se extrajo como texto.
...
| | GBM/SIC | IBKR Tiered, tasa marginal 20% (hipotética) | IBKR Tiered, tasa marginal 30% (hipotética) |
|---|---|---|---|
| Fricción de operación (§3.2) | 7.0% + 2.4%–6.0% = **9.4%–13.0%** | **3.1%** | **3.1%** |
| ISR sobre 3,000 MXN de ganancia | 10% = 300 MXN (1.5% de la cuenta) | ~600 MXN (3.0%) | ~900 MXN (4.5%) |
| **Total (puntos de la cuenta)** | **~10.9%–14.5%** | **~6.1%** | **~7.6%** |

- **[I]** Con alta rotación, el ahorro en fricción de IBKR supera el castigo fiscal, aun con tasa marginal alta. Con rotación baja (≤4 al año) y ganancias altas, el SIC puede ganar después de impuestos. **Esto no incluye el costo del contador ni de los pagos provisionales mensuales**, ni el riesgo de que el art. 126 exija 20% del monto de cada venta.

### 10.5 IVA
- **[H]** El listado de prestadores de servicios digitales inscritos en el RFC (Oficio 700-04-00-00-00-2025-060, DOF 25-jul-2025) **no incluye a Interactive Brokers**; sí incluye, por ejemplo, a TradingView [24].
...
- **[R] Plan y configuración si se abre la cuenta:**
  - IBKR Pro **Tiered**, cuenta de efectivo, fracciones activadas y datos gratis de Cboe One/IEX.
  - Depósito por SPEI una vez al mes.
  - Conversión automática (0.03%) o una sola conversión manual.
...
- **[I] Qué cambia en la estrategia:**
  - El universo de momentum se amplía (IMTM, IDMO, EEMO, PIE y cualquier ETF de EUA).
...
7. **Régimen fiscal mexicano de las ventas en NYSE/Nasdaq:** capítulo, pagos provisionales (art. 126 contra lo que permita el RLISR/RMF), compensación de pérdidas y tasa marginal. **Requiere contador.**
...
10. **ForecastTrader para residentes de México** (posible sustituto de Kalshi).
...
15. IBKR FAQ, "Has the $25,000 minimum equity requirement for pattern day traders changed?" (id 1460021712). https://www.interactivebrokers.com/lib/cstools/faq/#/content/1460021712 · FINRA, Regulatory Notice 26-10 (20-abr-2026). https://www.finra.org/rules-guidance/notices/26-10
...
34. Internas: `arena/investigacion/brokers/B1-gbm-linea-base-.md` (línea base de GBM, TC 17.71, patrón de operación), `arena/investigacion/brokers/B2-casas-de-bolsa-y-apps-mexicanas.md`, `arena/investigacion/01-gbm-operativa-y-costos.md`, `laboratorio/replicas/V05-spiva-y-fiscalidad-sic/prerregistro.md`.
