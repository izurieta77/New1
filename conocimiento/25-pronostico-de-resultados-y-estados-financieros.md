...
- **Walk-down hacia metas superables.**
  ...
  - Por eso la tasa de "beat" del S&P 500 es de **78%** (promedio de 5 años) y **76%** (10 años) [35]. **Sorpresa agregada promedio (adenda 2026-09-25, examen diagnóstico S5-09):** **7.0%** a 5 años y **7.4%** a 10 años [35]. "Supera el consenso" es el resultado base, no una señal; lo informativo es la magnitud frente a esa sorpresa típica, la guía o la reacción del precio.
...
- **Origen.** Bernard y Thomas (1989): el diferencial entre deciles extremos de SUE fue positivo en 41 de 48 trimestres (1974-1985) **(no verificado en fuente primaria)** y decrece con el tamaño [20]. Magnitud citada en la literatura (Bernard-Thomas 1989): cobertura de ≈**4.2% en 60 días hábiles** tras el anuncio (≈18% anualizado antes de costos).
- **Patrón multi-trimestre (adenda 2026-09-25, examen diagnóstico S5-09), Bernard y Thomas (1990), JAE 13:** los rendimientos anormales alrededor de los **siguientes 3 anuncios trimestrales** tienen el mismo signo que la sorpresa original, con magnitud decreciente, y el **4º anuncio** tiene **signo opuesto**. El patrón replica la autocorrelación de las diferencias estacionales de la utilidad (rezagos 1-3 positivos, rezago 4 negativo). Interpretación: los precios se comportan como si los inversionistas usaran un paseo aleatorio estacional (E_q = E_{q−4}) e ignoraran esa autocorrelación.
- **Chordia, Subrahmanyam y Tong (2014), JAE 58(1) (adenda 2026-09-25):** las anomalías de rendimiento (SUE incluida) se atenuaron con la mayor liquidez y la mayor actividad de arbitraje/trading, en línea con la decimalización de 2001; la caída es mayor en las acciones más líquidas.
...
- **Base normativa non-GAAP y eventos posteriores (adenda 2026-09-25, examen diagnóstico S5-12).** Lista de verificación con las normas primarias, ausente antes de esta adenda:
  - **C&DI 100.01 de la SEC (actualizado dic-2022):** excluir un gasto operativo normal, recurrente y en efectivo, necesario para operar el negocio, puede ser engañoso aunque se etiquete como "no recurrente"; "recurrente" incluye gastos a intervalos irregulares.
  - **C&DI 102.03 / Item 10(e) de la Regulation S-K:** no se puede llamar "no recurrente" a un cargo si hubo cargos similares en los dos años anteriores, o si es razonablemente probable que se repita en los próximos dos años.
  - **C&DI 102.11:** el efecto fiscal de una medida non-GAAP debe ser acorde con la propia medida, presentarse como un ajuste separado y explicarse; aplicar una tasa efectiva menor a la real sin sustento infla el EPS ajustado.
  - https://www.sec.gov/rules-regulations/staff-guidance/corporation-finance-interpretations/non-gaap-financial-measures
  - **ASC 855 (eventos posteriores):** si la condición que originó el cargo surgió **después** de la fecha del balance, es un evento posterior no reconocido (ASC 855-10-25-3; IAS 10 lo llama "no ajustable"): no se modifican los estados del periodo que cierra, pero se revela la naturaleza del evento y una estimación de su efecto, o se declara que no puede estimarse (ASC 855-10-50-2). El cargo se reconoce en el periodo siguiente.
  - **ASC 330-10-35-17 (pérdidas en compromisos de compra firmes y no cancelables):** si el precio de mercado cae por debajo del precio pactado en un compromiso de compra firme y no cancelable, la pérdida se reconoce en el periodo en que ocurre la caída de precio, como un **pasivo** con cargo a **costo de ventas** — es una norma distinta de la que rige el castigo del inventario ya en libros (ASC 330, valor neto de realización). https://viewpoint.pwc.com/content/pwc-madison/ditaroot/us/en/pwc/accounting_guides/utilities_and_power_/utilities_and_power__US/chapter_11_inventory_US/113_firm_purchase_co_US.html
  - **ASC 280-10-50-42:** obliga a revelar los clientes que representan 10% o más de los ingresos de la empresa.
