...
**Ambigüedad del divisor de Ω̂ (adenda 2026-09-25, examen diagnóstico S2-05).** Kamstra y Shi (2021), *"A Note on the GRS Test"*, UCR WP 202111, muestran que el artículo original de GRS (1989) es ambiguo sobre si la covarianza de los factores Ω̂ en el denominador (1 + μ̄'Ω̂⁻¹μ̄) debe llevar divisor T (máxima verosimilitud, la versión correcta para la distribución F exacta) o T−1 (insesgada). Verificado en el PDF del paper (texto extraído con `pypdf`): "using the incorrect formula leads to: (i) a test statistic that does not follow the F distribution as prescribed and over-rejects the null hypothesis of portfolio efficiency; (ii) **smaller models often being favored over larger ones when the statistic is used to rank asset pricing models**." Es decir, el divisor equivocado no solo infla el estadístico W, sino que **altera los *rankings* de modelos y favorece a los que tienen menos factores**. El efecto es despreciable con T grande, pero importa con N grande y T moderado. https://economics.ucr.edu/repec/ucr/wpaper/202111.pdf
